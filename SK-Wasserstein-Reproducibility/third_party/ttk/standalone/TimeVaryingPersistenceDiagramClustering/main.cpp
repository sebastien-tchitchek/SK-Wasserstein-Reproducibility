...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("TimeVaryingPersistenceDiagramClustering");

  // ---------------------------------------------------------------------------
  // Initialize ttkTimeVaryingPersistenceDiagramClustering module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto timeVaryingPersistenceDiagramClustering = vtkSmartPointer<ttkTimeVaryingPersistenceDiagramClustering>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // timeVaryingPersistenceDiagramClustering->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkTimeVaryingPersistenceDiagramClustering filter
      timeVaryingPersistenceDiagramClustering->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    timeVaryingPersistenceDiagramClustering->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkTimeVaryingPersistenceDiagramClustering filter
  // ---------------------------------------------------------------------------
  timeVaryingPersistenceDiagramClustering->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < timeVaryingPersistenceDiagramClustering->GetNumberOfOutputPorts(); i++) {
      auto output = timeVaryingPersistenceDiagramClustering->GetOutputDataObject(i);
