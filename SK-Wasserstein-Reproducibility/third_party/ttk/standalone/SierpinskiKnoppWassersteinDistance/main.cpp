...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("SierpinskiKnoppWassersteinDistance");

  // ---------------------------------------------------------------------------
  // Initialize ttkSierpinskiKnoppWassersteinDistance module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto sierpinskiKnoppWassersteinDistance = vtkSmartPointer<ttkSierpinskiKnoppWassersteinDistance>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // sierpinskiKnoppWassersteinDistance->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkSierpinskiKnoppWassersteinDistance filter
      sierpinskiKnoppWassersteinDistance->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    sierpinskiKnoppWassersteinDistance->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkSierpinskiKnoppWassersteinDistance filter
  // ---------------------------------------------------------------------------
  sierpinskiKnoppWassersteinDistance->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < sierpinskiKnoppWassersteinDistance->GetNumberOfOutputPorts(); i++) {
      auto output = sierpinskiKnoppWassersteinDistance->GetOutputDataObject(i);
