...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("TimeVaryingPersistenceDiagramDistanceMatrix");

  // ---------------------------------------------------------------------------
  // Initialize ttkTimeVaryingPersistenceDiagramDistanceMatrix module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto timeVaryingPersistenceDiagramDistanceMatrix = vtkSmartPointer<ttkTimeVaryingPersistenceDiagramDistanceMatrix>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // timeVaryingPersistenceDiagramDistanceMatrix->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkTimeVaryingPersistenceDiagramDistanceMatrix filter
      timeVaryingPersistenceDiagramDistanceMatrix->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    timeVaryingPersistenceDiagramDistanceMatrix->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkTimeVaryingPersistenceDiagramDistanceMatrix filter
  // ---------------------------------------------------------------------------
  timeVaryingPersistenceDiagramDistanceMatrix->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < timeVaryingPersistenceDiagramDistanceMatrix->GetNumberOfOutputPorts(); i++) {
      auto output = timeVaryingPersistenceDiagramDistanceMatrix->GetOutputDataObject(i);
