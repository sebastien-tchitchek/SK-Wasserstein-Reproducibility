...
void CGEDDistance( double &weight, double const &step, std::vector<std::vector<double>> const &costMatrix, std::vector<ttk::DiagramType> const &GeodesicSeti, std::vector<ttk::DiagramType> const &GeodesicSetj, bool assignements, bool withDiagonalMatching, std::vector<std::vector<int>> &matching, std::vector<double> &matchingsCost, double &distanceBetweenCurves, std::vector<double> &matchingsCostsWithDiagonalCostsIncluded){
            ...
            for(size_t q = 1; q < rows; q++) {
                ...
                idx(q,0)= idx(q-1,0)+(1-weight)*PersistenceOfCurvei[q-1]*(step);

            }

            for(size_t s = 1; s < cols; s++) {

                idx(0,s)= idx(0,s-1)+(1-weight)*PersistenceOfCurvej[s-1]*(step);
            ...
            for(size_t r = 1; r < rows; r++) {
                ...
                for(size_t t = 1; t < cols; t++) {
                    ...
                    double del = idx(r-1,t)+(1-weight)*PersistenceOfCurvei[r-1]*(step);
                    double add = idx(r,t-1)+(1-weight)*PersistenceOfCurvej[t-1]*(step);
            ...
            if(assignements == true){
                ...
                while(Mx != 0 || My != 0){
                    ...
                    if(Mx==0 && My != 0){
                        ...
                        double min2 = std::abs(idx(Mx,My)-idx(Mx-1,My)-(1-weight)*PersistenceOfCurvei[Mx-1]*(step));
                        double min3 = std::abs(idx(Mx,My)-idx(Mx,My-1)-(1-weight)*PersistenceOfCurvej[My-1]*(step));
                ...
                for(int i = 0; i < matching.size(); i++){
                    ...
                    if(a == -2 || b == -2){
                        ...
                        if(a == -2){
                            ...
                            matchingsCostsWithDiagonalCostsIncluded.push_back(PersistenceOfCurvej[b]*(step)*(1-weight));
                            
                        }
                        else if(b == -2){
                            
                            matchingsCostsWithDiagonalCostsIncluded.push_back(PersistenceOfCurvei[a]*(step)*(1-weight));
